    ...
    <div class="max-w-7xl mx-auto sm:w-8/12 md:w-1/2 lg:w-5/12">
      <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
          ...
          <form class="mb-6" action="{{ route('admin.carestation.update',$carestation->id) }}" method="POST">
            @method('put')
            @csrf
            <div class="flex flex-col mb-4">
              <label class="mb-2 uppercase font-bold text-lg text-grey-darkest" for="carestation_name">ステーション名</label>
              <input class="border py-2 px-3 text-grey-darkest" type="text" name="carestation_name" id="carestation_name" value="{{$carestation->carestation_name}}">
            </div>
            <div class="flex flex-col mb-4">
              <label class="mb-2 uppercase font-bold text-lg text-grey-darkest" for="address">住所</label>
              <input class="border py-2 px-3 text-grey-darkest" type="text" name="address" id="address" value="{{$carestation->address}}">
            </div>
            <div class="flex flex-col mb-4">
              <label class="mb-2 uppercase font-bold text-lg text-grey-darkest" for="tel">電話番号</label>
              <input class="border py-2 px-3 text-grey-darkest" type="text" name="tel" id="tel" value="{{$carestation->tel}}">
            </div>
            <div class="flex flex-col mb-4">
              <label class="mb-2 uppercase font-bold text-lg text-grey-darkest" for="fax">FAX番号</label>
              <input class="border py-2 px-3 text-grey-darkest" type="text" name="fax" id="fax" value="{{$carestation->fax}}">
            </div>
